...
func TestDsymmMatchesDense(t *testing.T) {
	...
	g := genericKernel{}
	for _, d := range []struct{ m, n int }{{1, 1}, {5, 3}, {17, 9}, {40, 25}} {
		for _, left := range []bool{true, false} {
			for _, upper := range []bool{true, false} {
				ta := d.n
				if left {
				...
				for j := 0; j < d.n; j++ {
					for i := 0; i < d.m; i++ {
						if math.Abs(c[i+j*ldc]-want[i+j*ldc]) > 1e-10*(1+math.Abs(want[i+j*ldc])) {
							t.Fatalf("symm m=%d n=%d L=%v U=%v (%d,%d): got %v want %v",
								d.m, d.n, left, upper, i, j, c[i+j*ldc], want[i+j*ldc])
						}
					}
				}
			}
						...
						for j := 0; j < d.n; j++ {
							for i := 0; i < d.m; i++ {
								if math.Abs(b[i+j*ldb]-want[i+j*ldb]) > 1e-9*(1+math.Abs(want[i+j*ldb])) {
									t.Fatalf("trmm m=%d n=%d L=%v U=%v T=%v unit=%v (%d,%d): got %v want %v",
										d.m, d.n, left, upper, trans, unit, i, j, b[i+j*ldb], want[i+j*ldb])
								}
							}
						}
					}
...
func TestDgerDtrsv(t *testing.T) {
	...
	for j := 0; j < n; j++ {
		for i := 0; i < m; i++ {
			want[i+j*lda] += alpha * x[i*2] * y[j]
		}
	}
	g.Dger(m, n, alpha, x, 2, y, 1, a, lda)
	for i := range a {
		if math.Abs(a[i]-want[i]) > 1e-12*(1+math.Abs(want[i])) {
			t.Fatalf("ger idx=%d: got %v want %v", i, a[i], want[i])
		}
	}

	// Dtrsv: residual check op(A)*x == b across all flag combos.
	for _, nn := range []int{1, 4, 13, 40} {
		for _, upper := range []bool{true, false} {
			for _, trans := range []bool{true, false} {
				for _, unit := range []bool{true, false} {
					lda := nn + 1
					...
					xv := make([]float64, nn)
					copy(xv, b)
					g.Dtrsv(upper, trans, unit, nn, ta, lda, xv, 1)
					// Verify op(A)*x == b.
					got := make([]float64, nn)
					dgemmNaive(trans, false, nn, 1, nn, 1, da, nn, xv, nn, 0, got, nn)
					for i := range b {
						if math.Abs(got[i]-b[i]) > 1e-8*(1+math.Abs(b[i])) {
							t.Fatalf("trsv n=%d U=%v T=%v unit=%v i=%d: residual %v want %v",
								nn, upper, trans, unit, i, got[i], b[i])
						}
					}
				}
			}
