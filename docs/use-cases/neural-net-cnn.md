...
`Sgemm`. That reshape is called **im2col**, and it is the reason a fast BLAS makes CNNs fast.
...
## The im2col trick: convolution → one Sgemm
...
```
Out = W · P        →  Sgemm, an (f × L) matrix
```

Row `i`, column `j` of `Out` is exactly "filter `i` applied at position `j`" — the whole
convolution, every filter at every location, as one goblas `Sgemm`. Reshape `Out` back to the
...
Images are float32, so this tutorial uses goblas's float32 matrix type,
[`mat32`](goblas-mat32-fundamentals.md) — no casting to float64, and convolution runs on the
`Sgemm` kernel.

Setup (no `blasadapt.Use()` needed — `mat32` calls the goblas single-precision kernels directly):
...
patches := mat32.NewDense32(c, L, nil)
col := 0
for oy := 0; oy < outH; oy++ {
    for ox := 0; ox < outW; ox++ {
        row := 0
...
```go
W := mat32.NewDense32(f, c, filterData) // each row is one flattened filter (float32)
```

**Step 3 — the convolution is one goblas `Sgemm`:**
...
fully-connected layers are `Sgemm`s exactly as in the [MLP tutorial](neural-net-mlp.md). So a
whole CNN's compute is: im2col + `Sgemm` per conv layer (the bulk), cheap pooling, and `Sgemm`
for the dense layers. **Backpropagation** through a conv layer is again matrix multiplies
(`col2im`, the reverse of im2col, plus `Sgemm`s for the gradients).

## The honest cost note

im2col itself (Step 1) is memory copying, not arithmetic, and it duplicates overlapping pixels —
so it trades extra memory for the ability to use a fast `Sgemm`. For typical filter sizes that
trade is overwhelmingly worth it, which is why it is the industry-standard approach. The copying
is plain Go and not accelerated by goblas; the `Sgemm` that follows — the actual `f·c·L`
arithmetic — is.
...
| Convolution `W·P` | `Sgemm` | accelerated — the bulk of the cost |
| Fully-connected layers | `Sgemm` | accelerated |
...
- **im2col** flattens every patch into the columns of a matrix; then the entire layer is one
  `Sgemm` (`W·P`) — the same approach real frameworks use.
- goblas accelerates that `Sgemm` (and the dense layers); im2col and pooling are cheap plain Go.
